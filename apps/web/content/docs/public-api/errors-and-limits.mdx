...
- `401 Unauthorized` for missing, malformed, expired, revoked, or invalid API keys.
- `403 Forbidden` for keys that cannot access the requested app or resource.
- `422 Unprocessable Entity` for enum or query validation failures.
...
- `meta.identityModel` is always `user`.
