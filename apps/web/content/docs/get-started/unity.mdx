...
**Pin a release tag** (`#v0.1.4` recommended). Unpinned URLs pull `main` and can break reproducible builds.
...
If any check fails, upgrade to `#v0.1.4` or newer, remove `Library/PackageCache/com.phase.analytics@*`, and re-import. Do not patch files inside `PackageCache` by hand.
...
| Distribution | npm | Git UPM (`#v0.1.4`) |
