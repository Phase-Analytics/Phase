...
If any check fails, upgrade to `#v0.1.11` or newer, remove `Library/PackageCache/com.phase.analytics@*`, and re-import. Do not patch files inside `PackageCache` by hand.
...
<TypeTable
  type={{
    ...
    UseUnityWebRequestTransport: {
      description: 'Use `UnityWebRequest` instead of `System.Net.Http` (main-thread marshaled). Leave `false` unless you need UWR specifically.',
      type: 'bool',
      default: 'false',
    },
    AllowInsecureDev: {
...
| Distribution | npm `phase-analytics` | Git UPM `#v0.1.11` |
