...
**Pin a release tag** (`#v0.1.2` recommended). Unpinned URLs pull `main` and can break reproducible builds.
...
If any check fails, upgrade to `#v0.1.2` or newer, remove `Library/PackageCache/com.phase.analytics@*`, and re-import. Do not patch files inside `PackageCache` by hand.
...
**Unity-specific:** the SDK does **not** send a `platform` field (`ios` / `android`). The server stores platform as unknown. Device OS is still available in metadata when `DeviceInfo` is enabled.
...
There is **no** automatic screen or scene tracking (unlike Expo / React Native with Expo Router). You control what is sent:
...
Use consistent event names (`scene_loaded`, `level_started`) so dashboards stay comparable across platforms.
...
- Use **Development** or **Release** IL2CPP build on hardware (Simulator can miss networking edge cases)
- Ensure App Transport Security allows your `BaseUrl` (default `https://api.phase.sh` is fine)
- Test offline: enable airplane mode, `Track` events, disable airplane mode, background app, verify flush in dashboard

  </TabsContent>

  <TabsContent value="android">

- Use IL2CPP **Release** on a physical device
- Confirm `INTERNET` permission (Unity adds it by default for network builds)
- Test offline flush the same way as iOS after reconnecting
...
|---------|---------------------|-------|
| Screen tracking | Automatic (optional) | Manual events only |
| `platform` field | Sent (`ios` / `android`) | Not sent |
| Distribution | npm | Git UPM (`#v0.1.2`) |
