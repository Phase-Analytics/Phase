...
If any check fails, upgrade to `#v0.1.7` or newer, remove `Library/PackageCache/com.phase.analytics@*`, and re-import. Do not patch files inside `PackageCache` by hand.
...
| Issue | What to check |
|-------|----------------|
| `Internal_CreateGameObject` / main thread | Fixed in `#v0.1.7+`. Upgrade; call `InitializeAsync` from main thread (`Start` / coroutine). Do not call `PhaseLifecycleHook.EnsureExists()` manually. |
...
| Distribution | npm `phase-analytics` | Git UPM `#v0.1.7` |
