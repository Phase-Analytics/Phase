...
**Pin a Unity tag** (`#unity-v0.1.6` recommended). Unity uses the `unity-v*` prefix. Expo / React Native npm releases use `v*` from `packages/sdk` (different versioning).

| Tag | What it fixes |
|-----|----------------|
| `v0.1.0`–`v0.1.4` or `unity-v0.1.0`–`unity-v0.1.4` | Same commits; prefer `unity-v*` going forward |
| `unity-v0.1.4` | Metas, compile fixes (`ValidationConstants`, `Logger`), UPM samples |
| `unity-v0.1.6` | `platform` ios/android, BCP47 locale, event-only API (no screen tracking) |

Intermediate tags `v0.1.1`–`v0.1.3`: asmdef `langVersion`, `csc.rsp`, `System.Threading` import.
...
If any check fails, upgrade to `#unity-v0.1.6` or newer, remove `Library/PackageCache/com.phase.analytics@*`, and re-import. Do not patch files inside `PackageCache` by hand.
...
**Platform:** on **iOS/Android player builds**, identify sends `platform: "ios"` or `"android"`. In the Editor it is `null`. **No screen tracking API** (unlike Expo Router auto-track or Swift `trackScreen`); use `Track` only, e.g. `Track("scene_loaded", ...)`.
...
There is **no** `TrackScreen` and no automatic navigation tracking. Model navigation with normal events:
...
| `CS0103` `ValidationConstants` | Upgrade to `#unity-v0.1.4`+ |
| `CS0104` ambiguous `Logger` | Upgrade to `#unity-v0.1.4`+ (`PhaseLifecycleHook`) |
...
|---------|---------------------|-------|
| Screen tracking | Automatic (optional, Expo Router) | **Not supported** — use `Track` events |
| `platform` field | Sent (`ios` / `android`) | Sent on mobile player builds (`unity-v0.1.6`+) |
| Distribution | npm tag `v*` | Git UPM tag `unity-v*` |
