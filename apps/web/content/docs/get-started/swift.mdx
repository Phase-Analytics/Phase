...
```swift
dependencies: [
    .package(url: "https://github.com/Phase-Analytics/Phase-Swift", from: "0.1.9")
]
```
</CodeBlock>
...
<TypeTable
  type={{
    ...
    deviceInfo: {
      description: 'Collect device metadata (model, OS version, platform, app version as `app_version`)',
      type: 'Bool',
      default: 'true',
    },
    userLocale: {
...
**Event parameters:**
- Flat primitive dictionary only
- Values must be `String`, `Int`, `Double`, `Bool`, or `nil`
- Max 32 keys
- Key max 32 characters
- String value max 256 characters
- Serialized payload max 8 KB
- Empty dictionaries are normalized away
...
<TypeTable
  type={{
    '[String: Any]': {
      description: 'Custom parameter name. Max 32 keys. Key max 32 chars. String values max 256 chars. Empty dictionaries are omitted.',
      type: 'String | Int | Double | Bool | nil',
    },
  }}
/>
