import type { ExploreFilter } from '@phase/shared';

const DEVICE_COLUMN_KEYS = new Set(['platform', 'country', 'city', 'locale']);

export function normalizeExploreFilterClient(
  filter: ExploreFilter
): ExploreFilter {
  if (filter.type === 'device_property' && DEVICE_COLUMN_KEYS.has(filter.key)) {
    return {
      type: 'device',
      field: filter.key as 'platform' | 'country' | 'city' | 'locale',
      operator: filter.operator === 'neq' ? 'neq' : 'eq',
      value: String(filter.value ?? '').trim(),
    };
  }

  return filter;
}

export function normalizeExploreFiltersClient(
  filters: ExploreFilter[]
): ExploreFilter[] {
  return filters.map(normalizeExploreFilterClient);
}
